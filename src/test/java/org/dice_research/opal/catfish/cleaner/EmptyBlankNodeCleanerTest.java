...
public class EmptyBlankNodeCleanerTest {
	...
	public void testEmptyBlankNodes() throws Exception {
		...
		new EmptyBlankNodeCleaner().processModel(testModel, null);

		Assert.assertTrue("EmptyBlankNodeCleaner removes empty blank node",
				testModel.size() < testCaseB.getModel().size());
